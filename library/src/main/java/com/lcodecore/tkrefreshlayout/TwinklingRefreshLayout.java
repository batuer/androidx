...
public class TwinklingRefreshLayout extends RelativeLayout implements PullListener, NestedScrollingChild {
    ...
    public TwinklingRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            mMaxHeadHeight = a.getDimensionPixelSize(R.styleable.TwinklingRefreshLayout_tr_max_head_height,
                (int)DensityUtil.dp2px(context, 120));
            mHeadHeight = a.getDimensionPixelSize(R.styleable.TwinklingRefreshLayout_tr_head_height,
                (int)DensityUtil.dp2px(context, 80));
            mMaxBottomHeight = a.getDimensionPixelSize(R.styleable.TwinklingRefreshLayout_tr_max_bottom_height,
                (int)DensityUtil.dp2px(context, 120));
            mBottomHeight = a.getDimensionPixelSize(R.styleable.TwinklingRefreshLayout_tr_bottom_height,
                (int)DensityUtil.dp2px(context, 60));
            mOverScrollHeight =
                a.getDimensionPixelSize(R.styleable.TwinklingRefreshLayout_tr_overscroll_height, (int)mHeadHeight);
            ...
            showRefreshingWhenOverScroll =
                a.getBoolean(R.styleable.TwinklingRefreshLayout_tr_showRefreshingWhenOverScroll, true);
            showLoadingWhenOverScroll =
                a.getBoolean(R.styleable.TwinklingRefreshLayout_tr_showLoadingWhenOverScroll, true);
        } finally {
            a.recycle();
        }
     ...
     * 拦截事件
     *
     * @return return true时,ViewGroup的事件有效,执行onTouchEvent事件 return false时,事件向下传递,onTouchEvent无效
     */
    @Override
    ...
    private boolean detectNestedScroll(MotionEvent e) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (index < 0) {
                    Log.e("TwinklingRefreshLayout", "Error processing scroll; pointer index for id " + mActivePointerId
