...
public class RoundProgressView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (oval == null)
            oval = new RectF();
        oval.set(getMeasuredWidth() / 2 - r, getMeasuredHeight() / 2 - r, getMeasuredWidth() / 2 + r,
        ...
        if (oval2 == null)
            oval2 = new RectF();
        oval2.set(getMeasuredWidth() / 2 - r - outCir_value, getMeasuredHeight() / 2 - r - outCir_value,
            getMeasuredWidth() / 2 + r + outCir_value, getMeasuredHeight() / 2 + r + outCir_value);// 设置个新的长方形，扫描测量
