...
public class ReflectUtils1 {
     ...
     * @param sourceClass
     *            给定的类
     * @param isGetDeclaredField
     *            是否需要获取Declared字段
     * @param isGetParentField
     *            是否需要把其父类中的字段也取出
     * @param isGetAllParentField
     *            是否需要把所有父类中的字段全取出
     * @param isDESCGet
     *            在最终获取的列表里，父类的字段是否需要排在子类的前面。只有需要把其父类中的字段也取出时此参数才有效
     * @return 给定类的所有字段
     */
     ...
     * @param clas
     *            给定的类
     * @param isGetDeclaredMethod
     *            是否需要获取Declared方法
     * @param isFromSuperClassGet
     *            是否需要把其父类中的方法也取出
     * @param isDESCGet
     *            在最终获取的列表里，父类的方法是否需要排在子类的前面。只有需要把其父类中的方法也取出时此参数才有效
     * @return 给定类的所有方法
     */
    public static List<Method> getMethods(Class<?> clas, boolean isGetDeclaredMethod, boolean isFromSuperClassGet,
        ...
        if (isFromSuperClassGet) {
            ...
            if (isDESCGet) {
                for (int w = classList.size() - 1; w > -1; w--) {
                    for (Method method : isGetDeclaredMethod ? classList.get(w).getDeclaredMethods()
     ...
     * @param sourceClass
     *            给定的类
     * @param isFindDeclaredConstructor
     *            是否查找Declared构造函数
     * @param isUpwardFind
     *            是否向上去其父类中寻找
        ...
        try {
            ...
            if (isUpwardFind) {
                ...
                while (method == null && classs != null) {
                    try {
                        method =
                            isFindDeclaredConstructor ? sourceClass.getDeclaredConstructor(constructorParameterTypes)
                                : sourceClass.getConstructor(constructorParameterTypes);
                    } catch (NoSuchMethodException e11) {
                        classs = classs.getSuperclass();
                    }
     ...
     *            是否需要把其父类中的构造函数也取出
     * @param isDESCGet
     *            在最终获取的列表里，父类的构造函数是否需要排在子类的前面。只有需要把其父类中的构造函数也取出时此参数才有效
     * @return 给定的类中所有的构造函数
     */
            ...
            if (isDESCGet) {
                for (int w = classList.size() - 1; w > -1; w--) {
                    for (Constructor<?> constructor : isFindDeclaredConstructor
                        ? classList.get(w).getDeclaredConstructors() : classList.get(w).getConstructors()) {
    ...
    public static <T> T getObjectByFieldName(Object object, String fieldName, Class<T> clas) {
        if (object != null && !StringUtils.isEmpty(fieldName) && clas != null) {
            try {
                Field field = ReflectUtils1.getField(object.getClass(), fieldName, true, true);
                if (field != null) {
                    field.setAccessible(true);
