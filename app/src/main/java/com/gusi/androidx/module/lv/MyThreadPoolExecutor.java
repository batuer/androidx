...
public class MyThreadPoolExecutor {
  private static final String TAG = "Fire_MyThreadPoolExecutor";
    ...
    public void setExecutor(ThreadPoolExecutor executor) {
      ...
      for (Field field : fields) {
        try {
          ...
          Log.i(TAG, field + " :--: " + field.get(executor));
        } catch (IllegalAccessException e) {
          Log.e(TAG, field + " :--: " + e.toString());
        }
      }
...
//      try {
//        Class<? extends ThreadPoolExecutor> aClass = executor.getClass();
//        Field workers = aClass.getDeclaredField("workers");
//        workers.setAccessible(true);
//        mWorkers = (HashSet) workers.get(executor);
//      } catch (Exception e) {
...
//      }
    ...
    public boolean offer(@NotNull Runnable runnable) {
      Log.i(TAG, runnable + " :--: " + mWorkers.size() + " : " + size());
      if (mWorkers.size() < 2) {
        return false;
      }
  ...
  public static class MyRunnable implements Runnable {
    ...
    public void run() {
      ...
      Log.w(TAG, Thread.currentThread().getName() + " : " + mFlag);
    }
