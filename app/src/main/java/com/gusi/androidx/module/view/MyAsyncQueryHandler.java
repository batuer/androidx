 ...
 */
public abstract class MyAsyncQueryHandler extends Handler {
    ...
    private final WeakReference<HandlerThread> mThreadWeakReference;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (event) {
                case EVENT_ARG_QUERY:
                    ...
                    try {
                        cursor =
                            resolver.query(args.uri, args.projection, args.selection, args.selectionArgs, args.orderBy);
    ...
    public MyAsyncQueryHandler(ContentResolver cr) {
        ...
        HandlerThread handlerThread = new HandlerThread("YlwAsync");
        handlerThread.start();
        mThreadWeakReference = new WeakReference<>(handlerThread);

        mWorkerThreadHandler = createHandler(sLooper);
    }

    public WeakReference<ContentResolver> getResolver() {
        return mResolver;
    }

    public WeakReference<HandlerThread> getThreadWeakReference() {
        return mThreadWeakReference;
    }
     ...
     *
     * @param token
     *            A token passed into {@link #onQueryComplete} to identify the query.
     * @param cookie
     *            An object that gets passed into {@link #onQueryComplete}
     * @param uri
     *            The URI, using the content:// scheme, for the content to retrieve.
     * @param projection
     *            A list of which columns to return. Passing null will return all columns, which is discouraged to
     ...
     * @param selectionArgs
     *            You may include ?s in selection, which will be replaced by the values from selectionArgs, in the order
    ...
    /**
     * Attempts to cancel operation that has not already started. Note that there is no guarantee that the operation
     * will be canceled. They still may result in a call to on[Query/Insert/Update/Delete]Complete after this call has
     * completed.
     *
     * @param token
     *            The token representing the operation to be canceled. If multiple operations have the same token they
     ...
     *
     * @param token
     *            A token passed into {@link #onInsertComplete} to identify the insert operation.
     * @param cookie
     ...
     *
     * @param token
     *            A token passed into {@link #onUpdateComplete} to identify the update operation.
     * @param cookie
     ...
     */
    public final void startUpdate(int token, Object cookie, Uri uri, ContentValues values, String selection,
     ...
     *
     * @param token
     *            A token passed into {@link #onDeleteComplete} to identify the delete operation.
     * @param cookie
     ...
     *
     * @param token
     *            the token to identify the query, passed in from {@link #startQuery}.
     * @param cookie
     ...
     *
     * @param token
     *            the token to identify the query, passed in from {@link #startInsert}.
     * @param cookie
     ...
     *
     * @param token
     *            the token to identify the query, passed in from {@link #startUpdate}.
     * @param cookie
     ...
     *
     * @param token
     *            the token to identify the query, passed in from {@link #startDelete}.
     * @param cookie
